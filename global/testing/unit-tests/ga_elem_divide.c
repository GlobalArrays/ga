/*
 * Test Program for GA
 * This is to test GA_Elem_divide (is a collective operation)
 * GA_Create -- used to create a global array using handles like 'g_A' 
 * GA_Elem_divide -- used to divide two array and store the quotient in different array 
 */
...
main(int argc, char **argv)
{
  ...
  int val_A=10, val_B=3, lo[DIM]={2,2}, hi[DIM]={4,4}, ld=DIM;
  ...
  if(rank==1)
    {
      /*
       *
       * This is one of the case : to compare the local array value
       *
       *
       for(i=0; i<DIM; i++)
       {
