/*
 * Test Program for GA
 * This is to test GA_Elem_minimum (is a collective operation)
 * GA_Create -- used to create a global array using handles like 'g_A' 
 * GA_Elem_minimum -- to find out the minimum out of two and store in different array
 */
...
main(int argc, char **argv)
{
  ...
  int val_A=5, val_B=9, lo[DIM]={2,2}, hi[DIM]={4,4}, ld=DIM, min;
  ...
  if(rank==1)
      ...
      for(i=0; i<DIM; i++)
	{
      ...
      */
      if(val_A>val_B) min=val_B;
      else min=val_A;

      for(i=0; i<DIM; i++)
	{
	  for(j=0; j<DIM; j++)
	    if(local_C[i][j]!=min) printf("GA Error : \n");
	}
    }
