/*
 * Test Program for GA
 * This is to test GA_Elem_minimum_patch (is a collective operation)
...
main(int argc, char **argv)
{
  ...
  int val_A=5, val_B=3, ld=DIM, min; 
  ...
  GA_Fill(g_B, &val_B);
  GA_Zero(g_C);
  GA_Elem_minimum_patch(g_A, lo, hi, g_B, blo, bhi, g_C, clo, chi);
  GA_Print(g_C);
  GA_Sync();
