...
#ifdef PERMUTE
if(GA_Proc_list)
    GAme = (Integer)GA_Proc_list[ga_msg_nodeid_()];
else
#endif
...
extern double t_dgop, n_dgop, s_dgop;
    ...
    GAinitialized = 0;
#ifdef PERMUTE0
    if(GAme==0) fprintf(stderr,"get statistics for node 0\n");
       ga_sync_();
    for(p=0;p<GAnproc;p++){
       ga_sync_();
       if(p==MPme){
          printf("p=%d| ",MPme);
          for(i=0;i<GAnproc;i++)printf("%d ",get_count[i]);
          printf("\n");
          fflush(stdout);
       }
    }
...
Integer proc;
   for( proc = 0; proc < *num_procs; proc++)
     #ifdef PERMUTE
       if(GA_Proc_list) 
           list[proc] = GA_inv_Proc_list[proc]; 
       else
     #endif
       list[proc]=proc;
}
