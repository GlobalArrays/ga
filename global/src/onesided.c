...
\*/
#define gaPermuteProcList(nproc,list)\
{\
  if((nproc) ==1) list[0]=0; \
  else{\
    int _i, iswap, temp;\
...
\
    /* initialize list */\
    for(_i=0; _i< (nproc); _i++) list[_i]=_i;\
\
		     ...
		     Integer *nbhandle) 
{
  ...
  int *ProcList = (int*)malloc(GAnproc*sizeof(int));
