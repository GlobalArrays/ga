              ...
              void *buf, Integer *ld)
{
  ...
  if (ga_property == READ_CACHE) //if array is read only
  {
    ...
    if (cache_head == NULL) {
        ...
        cache_head -> lo[0] = lo[0];
        cache_head -> hi[0] = hi[0];
        cache_head -> lo[1] = lo[1];
        cache_head -> hi[1] = hi[1];
        cache_head -> cache_buf = buf;
        ...
        //new
        memcpy(new_buf,buf,(sizeof(int)*(hi[0]-lo[0]+1)*(hi[1]-lo[1]+1)));
        
        cache_head -> cache_buf = new_buf;        
        ...
        while (cache_temp_pointer != NULL) {
           ...
           int temp_lo = cache_temp_pointer -> lo[0];
           int temp_lo2 = cache_temp_pointer -> lo[1];
           int temp_hi = cache_temp_pointer -> hi[0];
           int temp_hi2 = cache_temp_pointer -> hi[1];

           //temp_buff == buf
           if (/*temp_buf == buf*/  temp_lo == lo[0] && temp_hi == hi[0] && temp_lo2 == lo[1] && temp_hi2 == hi[1]) {

             //copy data to recieve buffer
             memcpy(buf,temp_buf,((sizeof(int)*(hi[0]-lo[0]+1)*(hi[1]-lo[1]+1))));
        ...
        cache_new = malloc(sizeof(cache_struct_t));
        cache_new -> lo[0] = lo[0];
        cache_new -> hi[0] = hi[0];
        cache_new -> lo[1] = lo[1];
        cache_new -> hi[1] = hi[1];      
        cache_new -> cache_buf = buf;
