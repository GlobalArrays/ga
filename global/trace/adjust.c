/*$Id: adjust.c,v 1.6 1998-04-18 01:30:41 d3h325 Exp $*/
/***********************************************************************\
...
int ar=record[1], et=record[6];
  ...
  stat[proc][ar+GA_OFFSET][et] += 1e-6 * (double)(t1-t0);
  acc[proc][ar+GA_OFFSET][et] ++;
}
...
long int i, ia, ip, ie;
  ...
  for(p=0;p<proc;p++){
     ...
     for(e=0; e<=event_types; e++){
        te =0.; ie=0;
        for(a=0;a<=arrays;a++){
           te += stat[p][a+GA_OFFSET][e];
           ie += acc[p][a+GA_OFFSET][e];
        }
     ...
     for(a=0; a<=arrays; a++){
        ta =0.; ia=0;
        for(e=0; e<=event_types; e++){
           ta += stat[p][a+GA_OFFSET][e];
           ia += acc[p][a+GA_OFFSET][e];
        }
