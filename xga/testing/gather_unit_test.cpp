...
int main(int argc, char **argv)
{
  ...
  for (i=0; i<idim; i++) {
    for (j=0; j<jdim; j++) {
      for (k=0; k<kdim; k++) {
        dptr[k+kdim*j+kdim*jdim*i] = static_cast<double>(k+lo[2] + (j+lo[1])*dims[2]
  ...
  for (n=rank; n<total; n+=size) {
    ...
    j = idx%dims[1];
    i = (idx-j)/dims[1];
    if (values[icnt] != static_cast<double>(k + j*dims[2]
          + i*dims[2]*dims[1])) {
      printf("p[%d] Check fails for i: %ld j: %ld k: %ld"
          " actual: %f expected: %f\n", rank,i,j,k,values[icnt],
          static_cast<double>(k + j*dims[2] + i*dims[2]*dims[1]));
      ok = 0;
    }
    icnt++;
  }
