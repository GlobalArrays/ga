...
#define XGA_FIND_PROC_INDICES_M(proc, index) {                             \
  int64_t _itmp, _i;                                                       \
  _itmp = proc;                                                            \
  index[p_ndim-1] = _itmp%p_proc_grid[p_ndim-1];                           \
  for (_i=p_ndim-2; _i>=0; _i--) {                                         \
    _itmp = (_itmp-index[_i+1])/p_proc_grid[_i+1];                         \
    index[_i] = _itmp%p_proc_grid[_i];                                     \
  }                                                                        \
}
...
#define XGA_FIND_BLOCK_FROM_INDICES_M(_iblock,index) {                     \
  ...
  for (_i=p_ndim-2; _i >= 0; _i--) {                                       \
    _iblock  = _iblock*p_proc_grid[_i]+index[_i];                          \
  }                                                                        \
}
...
#define XGA_FIND_PROC_FROM_SL_INDICES_M(proc,index) {                      \
  ...
  for (_i=0; _i<p_ndim; _i++) {                                            \
    _index2[_i] = index[_i]%p_proc_grid[_i];                               \
  }                                                                        \
  proc = _index2[0];                                                       \
  for (_i=1; _i<p_ndim; _i++) {                                            \
    proc = proc*p_proc_grid[_i]+_index2[_i];                               \
  }                                                                        \
}
...
#define XGA_FIND_TILE_PROC_FROM_SL_INDICES_M(proc,_index) {              \
  ...
  int64_t _index2[MAXDIM];                                               \
  for (_i=0; _i<p_ndim; _i++) {                                          \
    _index2[_i] = _index[_i]%p_proc_grid[_i];                            \
  }                                                                      \
  proc = _index2[0];                                                     \
  for (_i=1; _i <p_ndim; _i++) {                                         \
    proc = proc*p_proc_grid[_i]+_index2[_i];                             \
...
{                                                                        \
  ...
  for(_nb=1, _d=0; _d<_dim; _d++)_nb *= (int64_t)p_proc_grid[_d];        \
  if((int64_t)proc > _nb - 1 || proc<0){                                 \
    ...
    _index = proc;                                                       \
    for(_d=0; _d<_dim; _d++){                                            \
      _loc = _index% (int64_t)p_proc_grid[_d];                           \
      _index  /= (int64_t)p_proc_grid[_d];                               \
      _dimpos = _loc + _dimstart; /* find place in p_mapc */             \
      _dimstart += (int64_t)p_proc_grid[_d];                             \
      lo[_d] = (int64_t)p_mapc[_dimpos];                                 \
      if (_loc==p_proc_grid[_d]-1) hi[_d]=p_dims[_d]-1;                  \
...
{                                                                        \
  ...
  if( subscr[_d]<  lo[_d] ||  subscr[_d]>  hi[_d]){                      \
    char err_string[512];                                                \
    sprintf(err_string,"check subscript failed:%ld not in (%ld:%ld)"     \
        " dim=%d",                                                       \
