...
/* This function loads SUMMA arrays into GA arrays.  */
...
void ga_summa_to_ga( Integer my_row, Integer *kk_rows, Integer my_col, Integer *kk_cols,
...

{
  ...
  if( lst_col >= fst_col  && lst_row >= fst_row )
    ga_put_( ga_matrix, &fst_row, &lst_row, &fst_col, &lst_col, summa_matrix, &lda );
