...
void summa_abt( m, n, k, nb, alpha, a, lda, b, ldb, 
       ...
       *work1, *work2;  /* work arrays                             */
{
  ...
  for ( kk=0; kk<n; kk+=iwrk) {
    ...
    dgemm_( "No transpose", "Transpose", &m_c[ myrow ], &iwrk, &n_a[ mycol ],
...
void RING_SUM( buf, count, root, me, me_to, me_from, msg_id, work )

DoublePrecision   *buf, *work;
Integer  count, root, me, me_to, me_from, msg_id;

{

  static Integer    i_one=1;
  static DoublePrecision d_one=1.0;

  DoublePrecision      dummy;
  Integer     isize, isync, lenmes, nodefrom, ione, id_sync;
 
  id_sync = msg_id + 2;
  ione  = 1;
  isync = 1;
  isize = count * sizeof( DoublePrecision );

  if( me_from != root ) {
    SND_( &id_sync, (char *) &dummy, &ione , &me_from, &isync );
    RCV_( &msg_id, (char *) work, &isize, &lenmes, &me_from, &nodefrom, &isync );
    daxpy_( &count, &d_one, work, &i_one, buf, &i_one );
  }

  if( me != root ) {
    RCV_( &id_sync, (char *) &dummy, &ione, &lenmes, &me_to, &nodefrom, &isync );
    SND_( &msg_id, (char *) buf, &isize, &me_to, &isync );
  }
