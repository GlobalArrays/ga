...
void NextValueServer()
{
    ...
    Integer lencnt  = sizeof cnt;   /* length of cnt */
    Integer ndone   = 0;            /* no. finished for this loop */
    Integer ntermin = 0;            /* no. terminated so far (pend) */
    Integer node    = -1;           /* select any node */
    Integer type    = TYPE_NXTVAL;  /* message type */
    Integer buf[2];                 /* buffer to get values */
    Integer lenbuf  = sizeof buf;   /* length of buffer */
    Integer mproc;                  /* no. of processes running loop */
    Integer nval;                   /* no. of values requested */
    Integer done_list[MAX_PROCESS]; /* list of processes finished with this loop */
    Integer sync = 1;               /* all info goes synchronously */
    Integer on=0;
    Integer lenmes, nodefrom;

    SR_exit_on_error = FALSE;    /* Want to return no matter what */

    if (setjmp(SR_jmp_buf)) {    /* Error should Integer jump to here */
        /*    (void) printf("Error Integer jumped to NXTVAL ... returning.\n"); */
...
{
    Integer server = NNODES_();         /* id of server process */
    Integer buf[2];
    Integer lenbuf = sizeof buf;
    Integer type = TYPE_NXTVAL;
    Integer lenmes, nodefrom;
    Integer sync = 1;
    Integer result=0;
