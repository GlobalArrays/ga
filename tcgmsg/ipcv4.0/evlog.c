...
static DoublePrecision walltime();
 ...
 * Since the internal data format uses colons ':', DoublePrecision quotation
 ...
 *          EVKEY_STR_DBL, "Norm of matrix", (DoublePrecision) matrix_norm,
 *          EVKEY_LAST_ARG);
 *
 *    evlog(EVKEY_BEGIN, "Transform matrix",
 *          EVKEY_STR_DBL, "Recomputed norm", (DoublePrecision) matrix_norm,
 *      EVKEY_LAST_ARG);
 *      
 *
 ...
 *
 * EVKEY_STR_DBL, (char *) string, (DoublePrecision) data
 *      User data value pair (char *), (DoublePrecision)
 *
...
/**
 * return the wall time in seconds as a DoublePrecision.
 */
static DoublePrecision walltime()
{
    return ((DoublePrecision) MTIME_()) * 0.01;
}
...
/*
   ...
   {
   int N = 19;
   DoublePrecision matrix_norm = 99.1;
   int niters = 5;
