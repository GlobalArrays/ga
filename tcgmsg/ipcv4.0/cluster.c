 ...
 */
void InitClusInfo(char *procgrp, char *masterhostname)
{
    ...
    while (1) {
        ...
        if (!SR_clus_info[SR_n_clus].user
                || !SR_clus_info[SR_n_clus].hostname
                || !SR_clus_info[SR_n_clus].image
                || !SR_clus_info[SR_n_clus].workdir) {
...
void PrintClusInfo()
{
    ...
    printf("SR_clus_info = %ld size=%d\n",
            (Integer) SR_clus_info, (int)sizeof(struct cluster_info_struct));
    (void) fflush(stdout);
}
