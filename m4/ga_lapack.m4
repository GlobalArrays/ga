...
AS_IF([test $ga_lapack_ok = no],
    [ga_save_LIBS="$LIBS"; LIBS="$LAPACK_LIBS $BLAS_LIBS $LIBS"
...
AS_IF([test $ga_lapack_ok = yes],
    ...
    [AS_IF([test "x$enable_f77" = xyes],
        ...
        [AC_MSG_WARN([LAPACK library not found and Fortran is disabled])
         AC_MSG_WARN([LAPACK use is disabled])])
     $2])
