...
AC_DEFUN([TASCEL_MPI], [
# TASCEL_MPI_* vars might exist in environment, but they are really internal.
# Reset them.
TASCEL_MPI_LIBS=
TASCEL_MPI_LDFLAGS=
TASCEL_MPI_CPPFLAGS=
...
AS_IF([test x$need_parse = xyes],
    [TASCEL_ARG_PARSE([with_mpi],
        [TASCEL_MPI_LIBS], [TASCEL_MPI_LDFLAGS], [TASCEL_MPI_CPPFLAGS])])
AC_SUBST([TASCEL_MPI_LIBS])
AC_SUBST([TASCEL_MPI_LDFLAGS])
AC_SUBST([TASCEL_MPI_CPPFLAGS])
])dnl
