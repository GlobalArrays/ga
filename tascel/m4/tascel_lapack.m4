# TASCEL_F77_LAPACK_TEST
# ------------------
# Generate Fortran 77 conftest for LAPACK.
AC_DEFUN([TASCEL_F77_LAPACK_TEST], [AC_LANG_CONFTEST([AC_LANG_PROGRAM([],
[[      implicit none
      external DGETRS
      CALL DGETRS ()]])])
])
...
# TASCEL_C_LAPACK_TEST
# ----------------
# Generate C conftest for LAPACK.
AC_DEFUN([TASCEL_C_LAPACK_TEST], [AC_LANG_CONFTEST([AC_LANG_PROGRAM(
[#ifdef __cplusplus
extern "C" {
#endif
char dgetrs ();
...
# TASCEL_LAPACK([ACTION-IF-FOUND[, ACTION-IF-NOT-FOUND]])
# ---------------------------------------------------
# Originally from http://www.nongnu.org/autoconf-archive/tascel_lapack.html
# Modified to support many options to --with flag, updated to use AS_* macros,
# and different defaults for ACTIONs.
AC_DEFUN([TASCEL_LAPACK],
[AC_REQUIRE([TASCEL_BLAS])
AC_ARG_WITH([lapack],
    [AS_HELP_STRING([--with-lapack=[[ARG]]], [use external LAPACK library])])

tascel_lapack_ok=no
AS_IF([test "x$with_lapack" = xno], [tascel_lapack_ok=skip])

# Parse --with-lapack argument. Clear previous values first.
LAPACK_LIBS=
LAPACK_LDFLAGS=
LAPACK_CPPFLAGS=
TASCEL_ARG_PARSE([with_lapack], [LAPACK_LIBS], [LAPACK_LDFLAGS], [LAPACK_CPPFLAGS])
...
AS_IF([test $tascel_blas_ok != yes], [tascel_lapack_ok=noblas])
...
AS_IF([test $tascel_lapack_ok = no],
    [LIBS="$LAPACK_LIBS $BLAS_LIBS $LIBS"
     ...
     AC_MSG_RESULT([$tascel_lapack_ok])
     LIBS="$tascel_save_LIBS"])

# Generic LAPACK library?
for lib in lapack lapack_rs6k; do
AS_IF([test $tascel_lapack_ok = no],
    [tascel_save_LIBS="$LIBS"; LIBS="$BLAS_LIBS $LIBS"
     AC_CHECK_LIB([$lib], [$dgetrs],
        [tascel_lapack_ok=yes; LAPACK_LIBS="-l$lib"], [], [$FLIBS])
     LIBS="$tascel_save_LIBS"])
done
...
AS_IF([test $tascel_lapack_ok = yes],
    [have_lapack=1
     $1],
...
AM_CONDITIONAL([HAVE_LAPACK], [test $tascel_lapack_ok = yes])
])dnl TASCEL_LAPACK
