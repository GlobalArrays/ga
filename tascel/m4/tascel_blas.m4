# TASCEL_F77_BLAS_TEST
# ----------------
# Generate Fortran 77 conftest for BLAS.
AC_DEFUN([TASCEL_F77_BLAS_TEST], [AC_LANG_CONFTEST([AC_LANG_PROGRAM([],
[[      implicit none
...
AC_DEFUN([TASCEL_C_BLAS_TEST], [AC_LANG_CONFTEST([AC_LANG_PROGRAM(
[#ifdef __cplusplus
extern "C" {
#endif
char cgemm ();
...
# TASCEL_C_RUN_BLAS_TEST
# ------------------
# Test the C linker.
# Clears BLAS_LIBS on failure.  Sets tascel_blas_ok=yes on success.
AC_DEFUN([TASCEL_C_RUN_BLAS_TEST], [
   AC_LANG_PUSH([C])
   TASCEL_C_BLAS_TEST()
   AC_LINK_IFELSE([], [tascel_blas_ok=yes], [BLAS_LIBS=])
   AC_LANG_POP([C])
])dnl

# TASCEL_F77_RUN_BLAS_TEST
# --------------------
# Test the Fortran 77 linker.
# Clears BLAS_LIBS on failure.  Sets tascel_blas_ok=yes on success.
AC_DEFUN([TASCEL_F77_RUN_BLAS_TEST], [
...
# TASCEL_RUN_BLAS_TEST
# ----------------
# Test the linker.
# Clears BLAS_LIBS on failure.  Sets tascel_blas_ok=yes on success.
AC_DEFUN([TASCEL_RUN_BLAS_TEST], [
...
# TASCEL_BLAS([ACTION-IF-FOUND[, ACTION-IF-NOT-FOUND]])
# -------------------------------------------------
# Originally from http://www.nongnu.org/autoconf-archive/tascel_blas.html
...
tascel_blas_ok=no
AS_IF([test "x$with_blas" = xno], [tascel_blas_ok=skip])

# Parse --with-blas argument. Clear previous values first.
BLAS_LIBS=
BLAS_LDFLAGS=
BLAS_CPPFLAGS=
TASCEL_ARG_PARSE([with_blas], [BLAS_LIBS], [BLAS_LDFLAGS], [BLAS_CPPFLAGS])
...
tascel_save_LDFLAGS="$LDFLAGS";     LDFLAGS="$BLAS_LDFLAGS $LDFLAGS"
tascel_save_CPPFLAGS="$CPPFLAGS";   CPPFLAGS="$BLAS_CPPFLAGS $CPPFLAGS"
...
AS_IF([test $tascel_blas_ok = no],
    [AC_MSG_CHECKING([for BLAS in AMD Core Math Library])
     ...
     TASCEL_RUN_BLAS_TEST()
     LIBS="$tascel_save_LIBS"
     AS_IF([test "x$tascel_blas_ok" = xyes],
        [AS_IF([test $blas_size_hack = yes],
            [AS_CASE(["$BLAS_LIBS $LIBS $LDFLAGS $CPPFLAGS"],
                [*_int64*], [blas_size=8])])])
     AC_MSG_RESULT([$tascel_blas_ok])])
...
AS_IF([test $tascel_blas_ok = yes],
    ...
    [AC_MSG_WARN([BLAS library not found, using internal BLAS])
     blas_size=$tascel_cv_f77_integer_size # reset blas integer size to desired
     have_blas=0
     $2])
...
AM_CONDITIONAL([HAVE_BLAS], [test $tascel_blas_ok = yes])
])dnl TASCEL_BLAS
