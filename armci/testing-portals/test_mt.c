...
void test_pairs(int th_idx)
{
    ...
    for (i = 0; i < iters; i++) {
        ...
        /* get from my pair */
        assert(!PARMCI_Get(src, dst, ASIZE_BYTES, rem_proc));
    }
...
//            assert(!PARMCI_Put(src, dst, ASIZE_BYTES, b_proc));
            assert(!PARMCI_PutS(src, stride, dst, stride, count, 1, b_proc));
        }
        PARMCI_Fence(b_proc);
    }
    MT_BARRIER();
...
//return; // REMOVE WHEN DONE
    ...
    if (tgt != -1) {
        ...
        for (i = 0; i < iters; i++) {
            ...
            assert(!PARMCI_GetS(src, stride, dst, stride, count, 1, b_proc));
        }
    }
    ...
    if (tgt != -1) {
        ...
        for (i = 0; i < iters; i++) {
            src = &AELEM(ptrs1[a], b, i, 0); /* a.ptrs1[b] */
            dst = &AELEM(ptrs2[b], a, i, 0); /* b.ptrs2[a] */
            assert(!PARMCI_AccS(ARMCI_ACC_DBL,&scale,src,stride,dst,stride,count,1,b_proc));
        }
        PARMCI_Fence(b_proc);
    }
    MT_BARRIER();
