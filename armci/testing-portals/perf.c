...
/* tells to use PARMCI_Malloc_local instead of plain malloc */
#define MALLOC_LOC 
                ...
                int proc, int levels)
{
    ...
    for(i=0; i<loop; i++) {
        ...
        if(levels>0)for(j=0; j< count[1]; j++){
           ...
           PARMCI_Get(src_buf, dst_buf, count[0],proc);
        }
        else
#endif
        if(levels)
           PARMCI_GetS(src_buf, stride, dst_buf, stride, count, stride_levels,proc);
        else
           PARMCI_Get(src_buf, dst_buf,count[0], proc);

        if(CHECK_RESULT) {
            sprintf(check_type, "PARMCI_GetS:");
                ...
                int proc, int levels)
{
    ...
    for(i=0; i<loop; i++) {
        ...
        if(levels>0)for(j=0; j< count[1]; j++){
           char *s = (char*) src_buf, *d= (char*)dst_buf;
           s += j*stride[0]; d += j*stride[0];
           PARMCI_Put(src_buf, dst_buf, count[0],proc);
        }
        else
#endif
        if(levels)
           PARMCI_PutS(src_buf, stride, dst_buf, stride, count, stride_levels,proc);
        else
           PARMCI_Put(src_buf, dst_buf,count[0], proc);

        if(CHECK_RESULT) {
            PARMCI_GetS(dst_buf, stride, tmp_buf, stride, count,
                       stride_levels, proc);

            sprintf(check_type, "PARMCI_PutS:");
                ...
                int proc, int levels)
{
    ...
    for(i=0; i<loop; i++) {
        ...
        if(CHECK_RESULT) {
            PARMCI_GetS(dst_buf, stride, before_buf, stride, count,
                       stride_levels, proc);

            acc_array(scale, before_buf, src_buf, stride, count,stride_levels);
        }

        PARMCI_AccS(ARMCI_ACC_DBL, &scale, src_buf, stride, dst_buf, stride,
                   count, stride_levels, proc);

        if(CHECK_RESULT) {
            PARMCI_GetS(dst_buf, stride, after_buf, stride, count,
                       stride_levels, proc);
            
            sprintf(check_type, "PARMCI_AccS:");
...
void test_1D()
{
...
#ifdef MALLOC_LOC 
    if(me == 0) {
        buf = (double *)PARMCI_Malloc_local(SIZE * SIZE * sizeof(double));
        assert(buf != NULL);
    }
#else
    if(me == 0) {
    ...
    ierr = PARMCI_Malloc(ptr, (SIZE * SIZE * sizeof(double)));
    assert(ierr == 0); assert(ptr[me]);
    ierr = PARMCI_Malloc(get_ptr, (SIZE * SIZE * sizeof(double)));
    ...
    /* cleanup */
    PARMCI_Free(get_ptr[me]);
    PARMCI_Free(ptr[me]);
    
#ifdef MALLOC_LOC 
    if(me == 0) PARMCI_Free_local(buf);
#else
    if(me == 0) free(buf);
#endif
}
