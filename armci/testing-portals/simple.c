...
double t0,t1,tget=0,tnbget=0,tput=0,tnbput=0,tnbwait=0,t2=0;
    ...
    for(k=0;k<10;k++){
      ...
      PARMCI_Malloc((void **)myptrs[k],400000*LOOP*sizeof(double)); 
      for(i=0;i<LOOP;i++)myptrs[k][me][i]=me+0.414;
      MPI_Barrier(MPI_COMM_WORLD);
      for(i=0;i<LOOP;i++){
        PARMCI_Get(myptrs[k][(me+1)%nprocs]+i,myptrs[k][me]+i,sizeof(double),(me+1)%nprocs);
      ...
      for(i=0;i<LOOP;i++){
        ...
        t0 = MPI_Wtime(); 
        PARMCI_NbGet(myptrs[k][(me+1)%nprocs]+i,myptrs[k][me]+i,sizeof(double),(me+1)%nprocs,&nbh);
        t1 = MPI_Wtime(); 
        PARMCI_Wait(&nbh);
        t2 = MPI_Wtime();
        tnbget+=(t1-t0);
    ...
    for(k=0;k<10;k++)PARMCI_Free(myptrs[k][me]);
    MPI_Barrier(MPI_COMM_WORLD);
    PARMCI_Finalize();
    PARMCI_Finalize();
    MPI_Finalize();
