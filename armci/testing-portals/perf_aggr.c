...
void test_aggregate(int dryrun) {
	...
	for(j=start; j<end; j++) PARMCI_Wait(&hdl_put[j]);
      }
	...
	if((rc=PARMCI_NbPutV(&darr, 1, i, &hdl_put[i])))
	  ARMCI_Error("armci_nbputv failed\n",rc);
      }
      for(i=1; i<nproc; i++) PARMCI_Wait(&hdl_put[i]);
	...
	for(j=start; j<end; j++) {  
      ...
      for(i=1; i<nproc; i++) PARMCI_Wait(&aggr_hdl_put[i]);
	...
	darr.bytes = sizeof(double);
	darr.ptr_array_len = elems[1];
	if((rc=PARMCI_NbGetV(&darr, 1, i, &hdl_get[i])))
	  ARMCI_Error("armci_nbgetv failed\n",rc);
	PARMCI_Wait(&hdl_get[i]);
      }
	...
	for(j=start; j<end; j++) PARMCI_Wait(&hdl_get[j]);
      }
      ...
      for(i=1; i<nproc; i++) {
	for(j=start; j<end; j++) {  
	  PARMCI_NbGet(&dsrc[i][j], &ddst_get[me][i*elems[1]+j], bytes,
		      i, &aggr_hdl_get[i]);
	}
      }
      for(i=1; i<nproc; i++) PARMCI_Wait(&aggr_hdl_get[i]);
