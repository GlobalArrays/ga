...
void create_array(void *a[], int elem_size, int ndim, int dims[])
{
...
#ifdef NEWMALLOC
     {
        ...
        PARMCI_Memget(bytes, &meminfo[g_idx][me], 0);

        for(i=0; i<nproc; i++) 
           armci_msg_brdcst(&meminfo[g_idx][i], sizeof(armci_meminfo_t), i);

        for(i=0; i<nproc; i++) 
           a[i] = PARMCI_Memat(&meminfo[g_idx][i], 0);
        g_idx++;
     }
#else
...
void test_dim(int ndim)
{
	...
	for(i=0;i<LOOP;i++){
	    ...
	    count[0]   *= sizeof(double); /* convert range to bytes at stride level zero */
            ...
            (void)PARMCI_PutS((double*)a + idx1, strideA, (double*)b[proc] + idx2, strideB, count, ndim-1, proc);
...
/*            sleep(1);*/
            ...
            /* note that we do not need PARMCI_Fence here since
             * consectutive operations targeting the same process are ordered */
	    (void)PARMCI_GetS((double*)b[proc] + idx2, strideB, (double*)c + idx3, strideA,  count, ndim-1, proc);
...
int idx1=0, idx2=0, idx3=0;
    ...
    for(ndim=1;ndim<=MAXDIMS;ndim++){
       elems1*= dimsB[ndim-1];
       elems *= dimsA[ndim-1];
       rc = PARMCI_Malloc(b[ndim], sizeof(double)*elems1);
    ...
    for(ndim=1;ndim<=MAXDIMS;ndim++){
       ...
       if(ndim==1){
         (void)PARMCI_NbPut((double*)a[ndim]+idx1,(double*)b[ndim][proc]+idx2,
                           count[0], proc, (hdl_put+ndim));
       }else{
         (void)PARMCI_NbPutS((double*)a[ndim]+idx1,strideA,
    ...
    for(ndim=1;ndim<=MAXDIMS;ndim++){
       ...
       count[0]   *= sizeof(double); 
       if(ndim==1){
         (void)PARMCI_NbGet((double*)b[ndim][proc]+idx2,(double*)c[ndim]+idx3,
                           count[0], proc, (hdl_get+ndim));
       }else{
         (void)PARMCI_NbGetS((double*)b[ndim][proc]+idx2,strideB,
                          (double*)c[ndim]+idx3,
...
int lenpergiov;
    ...
    rc = PARMCI_Malloc(getsrc,sizeof(double)*nproc*GIOV_ARR_LEN*lenpergiov);
    assert(rc==0);assert(getsrc[me]);
    rc = PARMCI_Malloc(putdst,sizeof(double)*nproc*GIOV_ARR_LEN*lenpergiov);
    ...
    for(i=0;i<nproc-1;i++){
       ...
       if((rc=PARMCI_NbPutV(dsc,GIOV_ARR_LEN,dstproc,hdl_put+dstproc)))
         ARMCI_Error("putv failed",rc);
    }
    if(me==0){
       printf("\n\tNow veryfying the vector put data for correctness");
    }
    for(i=0;i<nproc;i++)if(i!=me)PARMCI_Wait(hdl_put+i);
    MP_BARRIER();
    PARMCI_AllFence();/*every one syncs after put */
    ...
    for(i=0;i<nproc-1;i++){
       ...
       if((rc=PARMCI_NbGetV(dsc,GIOV_ARR_LEN,dstproc,hdl_get+dstproc)))
         ARMCI_Error("putv failed",rc);
    }
    if(me==0){
       printf("\n\tNow veryfying the vector get data for correctness");
    }
    for(i=0;i<nproc;i++)if(i!=me)PARMCI_Wait(hdl_get+i);
    MP_BARRIER();
...
void test_acc(int ndim)
{
	...
	elems = 1;
        ...
        for(i=0;i<TIMES*nproc;i++){ 
            ...
            (void)PARMCI_AccS(ARMCI_ACC_DBL,&alpha,(double*)a + idx1, strideA, 
	...
	(void)PARMCI_GetS((double*)b[me] + idx2, strideB, (double*)c + idx1, strideA,  count, ndim-1, me);
...
/*            print_2D_double((double *)a, dimsA[0], loA, hiA);*/
            ...
            if((rc=PARMCI_PutV(dsc, mrc, proc)))ARMCI_Error("putv failed ",rc);
...
/*            print_2D_double((double *)b[proc], dimsB[0], loB, hiB);*/
            ...
            if((cols-1))if((rc=PARMCI_PutV(dsc, cols-1, proc)))
            ...
            if((rc=PARMCI_GetV(dsc, 1, proc)))ARMCI_Error("getv failed ",rc);
...
void test_fetch_add()
{
    ...
    rc = PARMCI_Malloc((void**)arr,bytes);
    assert(rc==0);
    MP_BARRIER();
    ...
    rc = PARMCI_Rmw(ARMCI_FETCH_AND_ADD, &val, arr[0], 1, 0);
    assert(rc==0);
...
void test_swap()
{
    ...
    for(i = 0; i< LOOP; i++){
          val = LOCKED;
          do{
            rc = PARMCI_Rmw(ARMCI_SWAP, &val, arr[0], whatever, 0);
            assert(rc==0);
          }while (val == LOCKED); 
          val++;
          rc = PARMCI_Rmw(ARMCI_SWAP, &val, arr[0], whatever, 0);
...
void test_memlock()
{
        ...
        for(j=0;j<10*TIMES;j++){ 
         for(i=0;i<TIMES*nproc;i++){ 
            ...
            assert(!PARMCI_Put(a, pstart, bytes, proc));
            assert(!PARMCI_Get(pstart, c, bytes, proc));
            assert(!PARMCI_Get(pstart, c, bytes, proc));
            armci_unlockmem();
...
void test_rput()
{
    ...
    MP_BARRIER();
    for(i=0; i<nproc; i++) {
      PARMCI_PutValueInt(10*(me+1), (void *)&idst[i][me], i);
      PARMCI_PutValueLong((long)10*(me+1), (void *)&ldst[i][me], i);
      PARMCI_PutValueFloat(10.01*(me+1), (void *)&fdst[i][me], i);
      PARMCI_PutValueDouble(10.001*(me+1), (void *)&ddst[i][me], i);
    }

    for(i=0; i<nproc; i++) {
      idst_get[i] = PARMCI_GetValueInt(isrc_get[i], i);
      ldst_get[i] = PARMCI_GetValueLong(lsrc_get[i], i);
      fdst_get[i] = PARMCI_GetValueFloat(fsrc_get[i], i);
      ddst_get[i] = PARMCI_GetValueDouble(dsrc_get[i], i);
    }
...
void test_aggregate() {
	...
	dst_ptr[k] = (void *)&ddst_put[i][me*elems[1]+j];
      ...
      if((rc=PARMCI_NbPutV(&darr, 1, i, &usr_hdl_put[i])))
	ARMCI_Error("armci_nbputv failed\n",rc);

      start = end; end = start + COUNT*NUMAGG;
      for(j=start; j<end; j+=NUMAGG) {  
	bytes = sizeof(double)*NUMAGG;
	if((rc=PARMCI_NbPutS(&dsrc[me][j], NULL, &ddst_put[i][me*elems[1]+j], NULL, 
      ...
      for(j=start; j<end; j+=NUMAGG) {  
	bytes = sizeof(double)*NUMAGG;
	if((rc=PARMCI_NbPut(&dsrc[me][j], &ddst_put[i][me*elems[1]+j], bytes,
			   i, &usr_hdl_put[i])))
    ...
    for(i=0; i<nproc; i++) PARMCI_Wait(&usr_hdl_put[i]);
	...
	dst_ptr[k] = (void *)&ddst_get[me][i*elems[1]+j];
      ...
      darr.bytes = NUMAGG*sizeof(double);
      darr.ptr_array_len = k;
      if((rc=PARMCI_NbGetV(&darr, 1, i, &usr_hdl_get[i])))
	ARMCI_Error("armci_nbgetv failed\n", rc);

      start = end; end = start + COUNT*NUMAGG;
      for(j=start; j<end; j+=NUMAGG) {  
	bytes = sizeof(double)*NUMAGG;
	if((rc=PARMCI_NbGetS(&dsrc[i][j], NULL, &ddst_get[me][i*elems[1]+j], NULL, 
      ...
      for(j=start; j<end; j+=NUMAGG) {  
	bytes = sizeof(double)*NUMAGG;
	if((rc=PARMCI_NbGet(&dsrc[i][j], &ddst_get[me][i*elems[1]+j], bytes,
			   i, &usr_hdl_get[i])))
	  ARMCI_Error("armci_nbget failed\n",rc);
      }
    }

    for(i=0; i<nproc; i++) PARMCI_Wait(&usr_hdl_get[i]);
...
void test_implicit() {
	...
	dst_ptr[k] = (void *)&ddst_put[i][me*elems[1]+j];
      ...
      darr.bytes = NUMAGG*sizeof(double);
      darr.ptr_array_len = k;
      if((rc=PARMCI_NbPutV(&darr, 1, i, NULL)))
	ARMCI_Error("armci_nbputv failed\n",rc);
	...
	dst_ptr[k] = (void *)&ddst_get[me][i*elems[1]+j];
      ...
      darr.bytes = NUMAGG*sizeof(double);
      darr.ptr_array_len = k;
      if((rc=PARMCI_NbGetV(&darr, 1, i, NULL)))
	ARMCI_Error("armci_nbgetv failed\n", rc);
