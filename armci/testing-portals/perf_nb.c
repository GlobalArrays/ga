...
void test_perf_nb(int dry_run) {
	...
	for(i=1; i<nproc; i++) {
	  stime=MP_TIMER();
	  for(j=0; j<ntimes; j++)
	    if((rc=PARMCI_Put(&dsrc[me][0], &ddst[i][me*elems[1]], bytes,i)))
	      ARMCI_Error("armci_nbput failed\n",rc);
	  t1 += MP_TIMER()-stime;
	}
      }
      MP_BARRIER(); PARMCI_AllFence(); MP_BARRIER();
	...
	for(i=1; i<nproc; i++) {
	  stime=MP_TIMER();    
	  for(j=0; j<ntimes; j++)
	    if((rc=PARMCI_Get(&dsrc[i][0], &ddst[me][i*elems[1]], bytes,i)))
	...
	for(i=1; i<nproc; i++) {
	  for(j=0; j<ntimes; j++) {
	    stime=MP_TIMER();    
	    if((rc=PARMCI_NbPut(&dsrc[me][0], &ddst[i][me*elems[1]], bytes,
			       i, &hdl_put)))
	      ARMCI_Error("armci_nbput failed\n",rc);
	...
	for(i=1; i<nproc; i++) {
	  for(j=0; j<ntimes; j++) {
	    stime=MP_TIMER();    
	    if((rc=PARMCI_NbGet(&dsrc[i][0], &ddst[me][i*elems[1]], bytes,
			       i, &hdl_get)))
	      ARMCI_Error("armci_nbget failed\n",rc);
      ...
      for(j=0; j<ntimes; j++) {
	stime=MP_TIMER();
	if((rc=PARMCI_AccS(ARMCI_ACC_DBL, &scale, &dsrc[me][0], &stride, 
	...
	MP_BARRIER();
      ...
      for(j=0; j<ntimes; j++) {
	stime=MP_TIMER();    
	if((rc=PARMCI_NbAccS(ARMCI_ACC_DBL, &scale, &dsrc[me][0], &stride, 
