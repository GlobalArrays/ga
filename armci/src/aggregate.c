...
/**
   issues:(5,6)
   *******
   1. 2 or more different handle for the same destination process( - done).
   
   2. exception thrown if the same handle is used  for 2 or more destination 
   process (opposite of issue 1) (done)
   
   3. throw exception if the same handle is used for get and put. (done)
   
   4. Using a handle initally for non-blocking put/get and then it is made 
   into an aggregate handle after a couple of reguest are completed.
   (check SET_AGGREGATE_HANDLE) and later for non-aggregation (check 
   UNSET_AGGREGATE_HANDLE) (done)
   
   5. If number of bytes of an incoming request(should have same tag and proc)
   is same, use the same index as jarek mentioned (particularly for 
   registered put - do with linked list)
   
   6. Make sure to have minimum exceptions (do not throw exception for 
   maximum outstanding aggregate requests). try to deal with that situation 
   
   7. There is a bug in SET_AGGREGATE_HANDLE (done)
   
   8. For an SMP node, perform blocking for a non-blocking operation.

   9. In armci_agg_save_giov_descriptor(), what if the incoming darr is > 4096.
   Make sure you handle this case.(done)

   10. Do some smartz like if the buffer space is more, then increase the 
     number of requests. (done)
   
   11. too much comments

   12. include strided put/get and take care if ptr_aray_len > 4096 (done)

   13. Think about this exception. (done)
   armci_die("_armci_agg_get_buffer_position: Insufficient aggregate buffer size");(issues 12,13,9 are the same)

   14. it is armci_ihdl_t and not armci_hdl_t.

   15. If size of single request/handle is > a fixed limit (say 128K/64K), 
   then complete the data transfer. Also if the totalsize of all the 
   requests is > 128/64K then complete the transfer. (this is crap)
   
   16. name buf_pos as buf_pos_left, and buf_pos_end as buf_pos_right.
*/
