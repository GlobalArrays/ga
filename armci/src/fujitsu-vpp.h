...
#define CopyPatchFromXX(src, ld_src, dst, ld_dst, blocks, bytes, proc){\
        int _iii, _stat=0, _idx;\
        char *ps=(char*)src, *pd=(char*)dst;\
printf("lds=%d ldd=%d bl=%d bytes=%d proc=%d\n",ld_src,ld_dst,blocks, bytes, proc); fflush(stdout);\
        if((blocks)>1){\
          for (_iii=0; _iii<(blocks); _iii++){\
             _idx = _iii%MAX_IDS;\
             if(_iii >= MAX_IDS)_stat += VPP_ReadDone(_id[_idx]);\
             _id[_idx]=VPP_StartRead((PROC)(proc),(ADDRP)ps,(ADDRP)pd,(bytes));\
             ps += (ld_src);\
             pd += (ld_dst);\
          }\
        ...
        }else _stat = VPP_Read((PROC)(proc),(ADDRP)(src),(ADDRP)(dst),(bytes));\
        if(_stat)MPLIB_TERMINATE;\
}

#define CopyPatchFromYY(src, ld_src, dst, ld_dst, blocks, bytes, proc){\
        int _iii, _stat=0, _bytes2copy=1;\
        char *ps=(char*)src, *pd=(char*)dst;\
        if((blocks)>1) {   for (_iii=0;_iii<(blocks);_iii++){\
             _id[0]= VPP_StartRead((PROC)(proc),(ADDRP)ps,(ADDRP)pd,(bytes));\
             ps += (ld_src);\
             pd += (ld_dst);\
          }\
...
#define CopyPatchFrom___(src, ld_src, dst, ld_dst, blocks, bytes, proc){\
        ...
        char *ps=(char*)src, *pd=(char*)dst;\
        if((blocks)>1)for (_iii=0;_iii<(blocks);_iii++){\
             _stat += VPP_Read((PROC)(proc),(ADDRP)ps,(ADDRP)pd,(bytes));\
             ps += (ld_src);\
             pd += (ld_dst);\
        }else _bytes2copy=(bytes);\
        _stat += VPP_Read((PROC)(proc),(ADDRP)(src),(ADDRP)(dst),_bytes2copy);\
