...
void ARMCIX_DCMF_AccReceiveComplete (ARMCIX_DCMF_AccNbInfo_t * nbinfo)
{
  ...
  ARMCIX_DCMF_SignalReceiveComplete (nbinfo->connection);
...
int ARMCIX_NbAccV (int datatype, void * scale, armci_giov_t * darr, int len, int proc, armci_ihdl_t nb_handle)
{
  ...
  dcmf->connection = &__connection[proc];
  dcmf->active += n;
  //fprintf (stderr, "ARMCIX_NbAccV() -- n=%d, malloc (%d)\n", n, sizeof(DCMF_Request_t) * n);

  __connection[proc].sequence.origin += n;
  __connection[proc].active += n;
  __global_connection.active += n;

  DCMF_Callback_t cb_wait = { ARMCIX_DCMF_NbOp_cb_done, nb_handle };
...
#warning remove this ARMCIX_Fence() and implement some sort of ack scheme.
  ...
  //nb_handle->implicit = NULL;
  armcix_dcmf_opaque_t * dcmf = (armcix_dcmf_opaque_t *) &nb_handle.cmpl_info;
  //armcix_dcmf_opaque_t dcmf;
  dcmf->connection = &__connection[proc];
  dcmf->active = n;

  //fprintf (stderr, "ARMCIX_AccS() -- dcmf.connection=%p, dcmf.active=%d, dcmf.tmp=%p, n=%d\n", dcmf.connection, dcmf.active, dcmf.tmp, n);
  ...
  //fprintf (stderr, "ARMCIX_AccS() -- __connection[%d].sequence.origin=%d, __connection[%d].active=%d, __global_connection.active=%d, n=%d\n", proc, __connection[proc].sequence.origin, proc, __connection[proc].active, __global_connection.active, n);
  ...
  //fprintf (stderr, "ARMCIX_AccS() -- after recurse\n");
                   ...
                   armci_ihdl_t nb_handle)
{
  ...
  //fprintf (stderr, "ARMCIX_NbAccS() -- __connection[%d].sequence.origin=%d, __connection[%d].active=%d, __global_connection.active=%d\n", proc, __connection[proc].sequence.origin, proc, __connection[proc].active, __global_connection.active);
