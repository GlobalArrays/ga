  ...
  for (i = 0; i < len; i++)
  {
    for (j = 0; j < darr[i].ptr_array_len; j++)
    {
      ...
      DCMF_Get ( &__get_protocol,
                 ...
                 proc,
                 darr[i].bytes,
                 (char *) darr[i].dst_ptr_array[j],
                 (char *) darr[i].src_ptr_array[j]);
    }
  }

  //fprintf (stderr, "ARMCIX_NbGetV() <<\n");
  DCMF_CriticalSection_exit  (0);
                   ...
                   armci_ihdl_t nb_handle)
{
  ...
  //fprintf (stderr, "ARMCIX_NbGetS() -- nb_handle=%p, stride_levels=%d, src_stride_arr=%p, dst_stride_arr=%p, __connection[%d].sequence.origin=%d, __connection[%d].active=%d, __global_connection.active=%d\n", nb_handle, stride_levels, src_stride_arr, dst_stride_arr, proc, __connection[proc].sequence.origin, proc, __connection[proc].active, __global_connection.active);
