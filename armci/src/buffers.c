/* $Id: buffers.c,v 1.30 2007-02-09 22:46:03 andriy Exp $    **/
...
int smallbuf_size = sizeof(buf_smext_t)*(MAX_SMALL_BUFS);
	...
	fflush(stdout);
     ...
     if (!(_armci_active_socks = malloc(sizeof(active_socks_t))))
         armci_die("buffers.c, malloc _armci_active_socks failed",0);
...
#if 0 && defined(SOCKETS)
            _armci_buf_state->table[tbl_idx].busy = 1;
#else
#warning buffer's busy flag is not supported yet
#endif
