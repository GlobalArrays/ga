...
void test_io_int()
{
  ...
  type = MT_INT;
  g_a = NGA_Create(type, ndim, dims, "a", chunk);
  if (!g_a) GA_Error("NGA_Create failed: a", 0);
  g_b = NGA_Create(type, ndim, dims, "b", chunk);
...
void test_io_dbl()
{
  ...
  int g_a, g_b, d_a;
  int i, j, k, req, loop, index, ndim=NDIM;
  int dlo[NDIM],dhi[NDIM];
  int glo[NDIM],ghi[NDIM];
  int elem, type, op;
  int me, nproc;
  int dims[NDIM], reqdim[NDIM], chunk[NDIM], ld[NDIM];
  logical status;
  char fname[200];
  long idum;
...
/*     error checking: (g_a - g_b)^2 */
  ...
  err = GA_Ddot(g_b, g_b);
  if (err != 0) {
    if (me == 0) printf("NDRA_Read error = %d\n\n",err);
  } else {
    if (me == 0) printf("OK\n\n");
  }
...
/*  now d_a is 2^NDIM times larger than g_a */
  ...
  for (j=0; j<loop; j++) {
    ...
    if (NDRA_Write_section(FALSE, g_a, glo, ghi, d_a, dlo, dhi, &req) != 0)
...
char **argv;
{
  ...
  double max_sz=100000000, max_disk=10000000000, max_mem=1000000;
